...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, ExoLLM
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y,batch_x_mark,batch_y_mark) in enumerate(train_loader):
                ...
                outputs =  self.model(batch_x,batch_x_mark)
                f_dim = -1 if self.args.features == 'MS' else 0
                #print(outputs.shape,batch_y.shape) # torch.Size([256, 336]) torch.Size([256, 96, 1])
                outputs = outputs[:, -self.args.pred_len:, f_dim:]
                batch_y = batch_y[:, -self.args.pred_len:, f_dim:].to(self.device)
                ...
                if (i + 1) % 100 == 0:
                    print("\titers: {0}, epoch: {1} | pred_loss: {2:.7f}".format(i + 1, epoch + 1, loss.item()))
